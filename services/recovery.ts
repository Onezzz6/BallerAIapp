...
const recoveryService = {
  ...
  async fetchTodayRecoveryData(userId: string, date: Date) {
    try {
      ...
      const recoveryRef = db.collection('users').doc(userId).collection('recovery').doc(dateStr);
  ...
  async fetchRecoveryHistory(userId: string) {
    try {
      ...
      const recoveryQuery = db.collection(`users/${userId}/recovery`)
        .where('lastUpdated', '>=', pastWeekStart.toISOString())
        .where('lastUpdated', '<', today.toISOString());
      
      const recoverySnapshot = await recoveryQuery.get();
  ...
  async fetchRecoveryPlan(userId: string, dateStr: string) {
    try {
      const planRef = db.collection('users').doc(userId).collection('recoveryPlans').doc(dateStr);
  ...
  setupRecoveryListener(userId: string, dateStr: string) {
    ...
    try {
      ...
      // Set up real-time listener for recovery data
      const unsubscribe = recoveryRef.onSnapshot((docSnapshot) => {
